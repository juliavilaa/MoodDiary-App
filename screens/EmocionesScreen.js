...
export default function EmocionesScreen({ navigation }) {
    ...
    const { registros, agregarEmocion, eliminarEmocion, editarEmocion } = useContext(EmocionesContext);
                        ...
                        <View style={styles.modalCard}>
                                ...
                                {CATALOGO_EMOCIONES.map((cat) => (
                                    <TouchableOpacity
                                        ...
                                        style={[
                                            ...
                                            tipoSeleccionado === cat.nombre && { backgroundColor: cat.color },
                                        ]}
                                    ...
                                    >
                                        <Ionicons
                                            name={cat.icono}
                                            size={14}
                                            color={tipoSeleccionado === cat.nombre ? '#fff' : cat.color === '#B8D4F0' ? '#4a6fa5' : cat.color}
                                        />
                                        <Text style={[
                                            styles.tipoPillTexto,
                                            { color: tipoSeleccionado === cat.nombre ? '#fff' : '#555' }
                                        ]}>
                                            {cat.nombre}
                                        </Text>
                                    </TouchableOpacity>
                            ...
                            <View style={styles.modalBotones}>
                                <TouchableOpacity
                                    ...
                                    onPress={() => { setModalVisible(false); setDescripcion(''); setTipoSeleccionado(null); }}
                                >
                                    <Text style={styles.modalBotonCancelarTexto}>Cancelar</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    style={[styles.modalBotonGuardar, (!descripcion.trim() || !tipoSeleccionado) && { opacity: 0.5 }]}
...
const styles = StyleSheet.create({
    ...
    saludo: {
        ...
        marginTop: 8, marginBottom: 20, lineHeight: 30,
    },

    // Botón agregar
    agregarCard: {
        borderWidth: 1.5, borderColor: 'rgba(180,130,220,0.4)',
    ...
    modalInput: {
        ...
        borderRadius: 14, padding: 12, fontSize: 14,
        color: '#333', textAlignVertical: 'top', minHeight: 80,
    },
